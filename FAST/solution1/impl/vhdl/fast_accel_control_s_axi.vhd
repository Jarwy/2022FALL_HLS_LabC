...
entity fast_accel_control_s_axi is
generic (
...
end entity fast_accel_control_s_axi;
...
--        bit 31~0 - img_out[31:0] (Read)
-- 0x24 : Control signal of img_out
--        bit 0  - img_out_ap_vld (Read/COR)
--        others - reserved
...
architecture behave of fast_accel_control_s_axi is
    type states is (wridle, wrdata, wrresp, wrreset, rdidle, rddata, rdreset);  -- read and write fsm states
    signal wstate  : states := wrreset;
    signal rstate  : states := rdreset;
    signal wnext, rnext: states;
    constant ADDR_IMG_IN_DATA_0    : INTEGER := 16#10#;
    constant ADDR_IMG_IN_CTRL      : INTEGER := 16#14#;
    constant ADDR_THRESHOLD_DATA_0 : INTEGER := 16#18#;
    constant ADDR_THRESHOLD_CTRL   : INTEGER := 16#1c#;
    constant ADDR_IMG_OUT_DATA_0   : INTEGER := 16#20#;
    constant ADDR_IMG_OUT_CTRL     : INTEGER := 16#24#;
    constant ADDR_ROWS_DATA_0      : INTEGER := 16#30#;
    constant ADDR_ROWS_CTRL        : INTEGER := 16#34#;
    constant ADDR_COLS_DATA_0      : INTEGER := 16#38#;
    constant ADDR_COLS_CTRL        : INTEGER := 16#3c#;
    ...
    -- internal registers
    signal int_img_in          : UNSIGNED(31 downto 0) := (others => '0');
    signal int_threshold       : UNSIGNED(31 downto 0) := (others => '0');
    signal int_img_out_ap_vld  : STD_LOGIC;
    signal int_img_out         : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_IMG_OUT_CTRL =>
                        rdata_data(0) <= int_img_out_ap_vld;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_img_out <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (img_out_ap_vld = '1') then
                    int_img_out <= UNSIGNED(img_out);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_img_out_ap_vld <= '0';
            elsif (ACLK_EN = '1') then
                if (img_out_ap_vld = '1') then
                    int_img_out_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_IMG_OUT_CTRL) then
                    int_img_out_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
