...
typedef struct {
    ...
    XCordiccart2pol_uio_map maps[ MAX_UIO_MAPS ];
} XCordiccart2pol_uio_info;

/***************** Variable Definitions **************************************/
static XCordiccart2pol_uio_info uio_info;
...
static int uio_info_read_name(XCordiccart2pol_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XCordiccart2pol_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XCordiccart2pol_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XCordiccart2pol_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XCordiccart2pol_Initialize(XCordiccart2pol *InstancePtr, const char* InstanceName) {
	XCordiccart2pol_uio_info *InfoPtr = &uio_info;
